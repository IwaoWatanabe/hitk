...
正規化したパスを返す"""
  ...
  def get_entries(self, pattern='', dirOnly=False, ignoreCase=False):
    ...
    if pattern:
      ...
      gre = glob2re(pattern, ignoreCase=ignoreCase)
...
- copy file """
    ...
    while argv:
      opts, args = cli.getopt(argv, 'Rpnv', (
...
- find directory entry"""
  # ディレクトリを探索して、条件に合致するファイルをレポートする
    ...
    params = []
    while argv:
      opts, args = cli.getopt(argv, 't:d:u:g:n:x:m:s:',(
        ...
        with open(target) as fp:
            ...
            try:
                while line:
                    ...
                    if pflag:
                        if lnumber:
                            cli._print('%s: %s: %s' % (target, ct, line), end='')
                        else:
                            cli._print( '%s: %s' % (target, line), end='')

                    line = fp.readline()
            except Exception as e: 
                cli._print('ERROR: %s:%s:%s' % (tf, ct, e), file=cli.syserr)
                    
        for tf in cli.glob(args[ind:]):
            _grep_file(tf)

  def __init__(self):
    cli.CommandDispatcher.__init__(self)
    self.home = os.getcwd()
...
- echo back argument """
    ...
    sep = ''
    for tt in argv:
        cli._print(sep, file=cli.out, end='')
        sep = ' '
    cli._print(tt, file=cli.out, end='')
    cli._print(file=cli.out)
...
- echo test for alert  """
    ...
    cli._print('verbose:', cli.verbose, file=cli.syserr)
...
- print local work directory """
  ...
  def _lls(self, dirname='', detail=1, recursive=0, all=False):
    ...
    if not detail:
      ...
      if dirname: cli._print('%s:' % dirname)
      for ent in entries:
